...
Base.show(io::IO, x::Basis) = print(io, "$(length(x.basis)) dimensional basis in ", "$([v for v in x.variables])")
...
function Basis(basis::Function, variables::AbstractVector; parameters::AbstractArray =  [], iv = nothing, kwargs...)
    vrs = value.(variables)
    @assert all(is_independent.(vrs)) "Please provide independent variables for basis."
...
#
#    if eval_expression
#        f_oop, f_iip = eval.(ModelingToolkit.build_function(basis.basis, vs, ps, [basis.iv], conv=toexpr∘simplify))
#    else
#        f_oop, f_iip = ModelingToolkit.build_function(basis.basis, vs, ps, [basis.iv], conv=toexpr∘simplify)
#    end
#
#    f_(u, p, t) = f_oop(u, p, t)
#    f_(du, u, p, t) = f_iip(du, u, p, t)
#
#    b.f_ = ModelingToolkit.RuntimeGeneratedFunctions.@RuntimeGeneratedFunction(f_)
#    return
#end
#
#
...
#end
#
#function Base.push!(b::Basis, op::Sym)
...
#
#    if eval_expression
#        f_oop, f_iip = eval.(ModelingToolkit.build_function(expand_derivatives.(j), vs, ps, [basis.iv],conv=toexpr∘simplify))
#    else
#        f_oop, f_iip = ModelingToolkit.build_function(expand_derivatives.(j), vs, ps, [basis.iv], conv=toexpr∘simplify)
#    end
#
#    f_(u, p, t) = f_oop(u, p, t)
#    f_(du, u, p, t) = f_iip(du, u, p, t)
#
#    return ModelingToolkit.RuntimeGeneratedFunctions.@RuntimeGeneratedFunction(f_)
#end
#
#
